...
def fig_rms_distributions(data: dict, out_dir: Path) -> None:
    for mod, sr in [("seismic", 100), ("audio", 16000)]:
        ...
        for ax, (fname, df) in zip(axes_flat, mod_files.items(), strict=False):
...
def fig_spectral_density(data: dict, out_dir: Path) -> None:
    ...
    for ax, (fname, df) in zip(axes_flat, seismic_files.items(), strict=False):
        amp = df["amplitude"].values.astype("float64")
        pres = df["present"].values
        n_win = len(amp) // sr
        wins = amp[: n_win * sr].reshape(n_win, sr)
...
def print_statistics(data: dict, out_dir: Path) -> None:
    ...
    lines.append(
        f"Vehicle present:      {total_present:>15,}  ({total_present/total_samples*100:.1f}%)"
    )
    ...
    for mod, sr in [("seismic", 100), ("audio", 16000)]:
        ...
        lines.append(f"{'─'*72}")
        lines.append(
            f"  {'Recording':<38} {'Windows':>7} {'Present':>8} {'Bg':>8} {'%Pres':>6} {'SNR dB':>7} {'NF (RMS)':>9}"
        )
