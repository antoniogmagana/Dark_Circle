...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        "When set (default), train/val/test assignments come from "
    ...
    p.add_argument(
        "--init-search-root",
        default="saved_crl/runs",
        help="Root dir searched by --init-from-run=auto. Default: " "saved_crl/runs.",
    )
    p.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Master RNG seed (default 42). Seeds Python random, "
        "numpy, torch CPU+CUDA, and DataLoader shuffle/workers. "
...
def main() -> None:
    ...
    if args.crl_run_dir is not None:
        ...
        if not src_ckpt.exists():
            raise FileNotFoundError(f"--crl-run-dir missing {args.ckpt_name}: {src_ckpt}")
    ...
    if args.use_id_split:
        print(
            f"INFO: --use-id-split is set; --data-dir and --val-dir are ignored, "
    ...
    meta: dict = {
        ...
        "stage2": stage2,
        "init_from_run": (
            str(init_from_run_resolved.resolve()) if init_from_run_resolved is not None else None
        ),
        "seed": args.seed,
    }
