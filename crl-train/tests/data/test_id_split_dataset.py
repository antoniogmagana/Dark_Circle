...
def _write_split_runs_parquet(path, scene_run_lengths):
    ...
    for s, r, n in scene_run_lengths:
        for _ in range(n):
            rows.append({"scene_id": s, "run_id": r, "amplitude": 0.0, "present": True})
...
def id_root_split(tmp_path, monkeypatch):
    ...
    _write_simple_parquet(
        data_root / "train" / "iobt_audio_silverado_rs1.parquet",
        n_samples=TEST_AUDIO_W * 10,
    )
    _write_simple_parquet(
        data_root / "train" / "iobt_seismic_silverado_rs1.parquet",
...
class TestIdSplitTrainMarker:
    def test_train_marker_routes_all_to_train(self, tmp_path, small_cfg, monkeypatch):
        ...
        _write_simple_parquet(
            data_root / "train" / "iobt_audio_polaris_rs1.parquet",
            n_samples=TEST_AUDIO_W * 6,
        )
        _write_simple_parquet(
            data_root / "train" / "iobt_seismic_polaris_rs1.parquet",
...
class TestIdSplitRunsMarker:
    def test_split_runs_partitions_across_three_roles(self, tmp_path, small_cfg, monkeypatch):
        ...
        seismic_runs = [
            (1, 6, TEST_SEISMIC_W * 4),
            (1, 7, TEST_SEISMIC_W * 4),
            (2, 6, TEST_SEISMIC_W * 4),
        ]
        _write_split_runs_parquet(data_root / "train" / "m3nvc_audio_cx30_rs1.parquet", audio_runs)
        _write_split_runs_parquet(
            data_root / "train" / "m3nvc_seismic_cx30_rs1.parquet", seismic_runs
        )

        lengths = {}
        for role in ("train", "val", "test"):
            ds = SensorDataset(
                parquet_dir=data_root / role,
                config=small_cfg,
                is_train=(role == "train"),
...
class TestIdSplitCli:
    ...
    def test_use_id_split_default_true(self, monkeypatch):
    ...
    def test_no_use_id_split_disables_flag(self, monkeypatch):
        sys.path.insert(0, str(Path(__file__).parents[2]))
        import train

        monkeypatch.setattr("sys.argv", ["train.py", "--no-use-id-split"])
        args = train.parse_args()
