...
def _synthetic_batch(B=4, n_partners=4):
    ...
    if n_partners > 0:
        batch["x_audio_partners"]   = torch.randn(B, n_partners, 1, W_a) * 0.01
        batch["x_seismic_partners"] = torch.randn(B, n_partners, 1, W_s) * 0.01
        ...
        batch["partner_stratum_partners"] = torch.stack(
            [torch.full((B,), p % 4) for p in range(n_partners)], dim=1
        )
    return batch
