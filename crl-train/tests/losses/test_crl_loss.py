...
class TestKLDivergence:
    def test_zero_at_prior(self):
        assert kl_divergence(torch.zeros(8, 24), torch.zeros(8, 24)).item() == pytest.approx(
            0.0, abs=1e-5
        )
...
class TestInterventionMatchingLoss:
    def test_scalar(self):
        assert (
            intervention_matching_loss(torch.randn(8, 2), torch.randint(0, 2, (8, 2)).float()).shape
            == ()
        )
    ...
    def test_matches_bce_with_logits(self):
        ...
        assert intervention_matching_loss(logits, targets).item() == pytest.approx(
