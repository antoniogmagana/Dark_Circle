...
@pytest.fixture
def latent():
    return CausalLatentSpace(d_z=32)


def test_class_constants():
    assert CausalLatentSpace.D_PRES == 4
    assert CausalLatentSpace.D_TYPE == 12
    assert CausalLatentSpace.D_PROX == 3
    assert CausalLatentSpace.D_ENV  == 6
    assert CausalLatentSpace.D_CAUSAL == 25


def test_default_d_z_gives_d_free_7(latent):
    assert latent.d_z == 32
    assert latent.d_free == 7
...
def test_split_shapes(latent):
    ...
    assert z_type.shape == (8, 12)
    assert z_prox.shape == (8, 3)
    assert z_env.shape  == (8, 6)
    assert z_free.shape == (8, 7)


def test_split_contiguous_partition(latent):
    z = torch.arange(32, dtype=torch.float).unsqueeze(0)
    z_pres, z_type, z_prox, z_env, z_free = latent.split(z)
    assert z_pres[0, -1].item() == 3.0    # dims 0-3
    assert z_type[0,  0].item() == 4.0    # dims 4-15
    assert z_prox[0,  0].item() == 16.0   # dims 16-18
    assert z_env[0,   0].item() == 19.0   # dims 19-24
    assert z_free[0,  0].item() == 25.0   # dims 25-31


def test_split_3d_input(latent):
    z = torch.zeros(4, 3, 32)
    z_pres, *_ = latent.split(z)
    assert z_pres.shape == (4, 3, 4)


def test_larger_d_z_expands_only_free_subspace():
    latent = CausalLatentSpace(d_z=40)
    assert latent.d_z == 40
    assert latent.d_free == 15

    z = torch.zeros(2, 40)
    z_pres, z_type, z_prox, z_env, z_free = latent.split(z)
    assert z_pres.shape == (2, 4)
    assert z_type.shape == (2, 12)
    assert z_prox.shape == (2, 3)
    assert z_env.shape  == (2, 6)
    assert z_free.shape == (2, 15)


def test_d_z_at_or_below_causal_budget_raises():
    with pytest.raises(ValueError, match="d_z > 25"):
        CausalLatentSpace(d_z=25)
    with pytest.raises(ValueError, match="d_z > 25"):
        CausalLatentSpace(d_z=24)
