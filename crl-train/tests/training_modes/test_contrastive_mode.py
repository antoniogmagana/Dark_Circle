...
from crl_vehicle.training_modes import (
    CheckpointState, ContrastiveTrainingMode, build_training_mode,
)
...
def _pair_batch(B=4, n_partners=4, audio_W=16000, seismic_W=100,
    ...
    if strata is None:
        strata = [STRATUM_CONSEC, STRATUM_SAME_TYPE, STRATUM_DIFF_TYPE, STRATUM_CROSS_DS]
    batch = {
...
class TestCheckpointSelection:
    def test_saves_on_improvement(self, cfg_ms):
        ...
        saves = mode.should_save_checkpoint(
            {"val_contrastive_loss": 5.0}, epoch=0, state=state,
        )
    ...
    def test_skips_on_no_improvement(self, cfg_ms):
        ...
        saves = mode.should_save_checkpoint(
            {"val_contrastive_loss": 5.5}, epoch=1, state=state,
        )
