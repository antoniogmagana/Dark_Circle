...
def plot_best_f1_bar(runs: list[A.RunMetrics], out: Path) -> None:
    ...
    patches = [plt.Rectangle((0, 0), 1, 1, color=c, alpha=0.8, label=fe) for fe, c in seen.items()]
    ax.legend(
        handles=[
            *patches,
            plt.Line2D(
...
def plot_complexity_vs_f1(runs: list[A.RunMetrics], out: Path) -> None:
    ...
    for ep, f1, rm in points:
        ...
        ax.annotate(rm.name, (ep, f1), fontsize=7, xytext=(4, 4), textcoords="offset points")
