...
from crl_config import (
    CLASS_MAP, CLASS_NAMES, Z_VEH_DIM, Z_ENV_DIM,
    MODALITY_FEATURE_DIM, BATCH_SIZE, NUM_WORKERS,
)
...
def run_classification_eval(
    ...
    metrics = {
        ...
        "per_class_f1":  f1_score(
            ...
            labels=target_labels, zero_division=0,
        ).tolist(),
    }
    ...
    lines += [
        ...
        classification_report(
            all_true, all_preds,
            labels=target_labels, target_names=CLASS_NAMES,
            zero_division=0,
        ),
        "Confusion Matrix (rows=true, cols=pred):",
...
def write_summary(results: dict, save_dir: Path):
    ...
    fieldnames = [
        "eval_set", "task",
        "accuracy", "f1_weighted", "macro_f1",
        "precision", "recall", "auc", "far", "mcc",
    ]
    rows = []
...
def parse_args():
    ...
    p.add_argument("--save-dir",  default="./saved_crl", help="Directory with saved model weights")
    p.add_argument("--eval-dir",  default="./eval_crl",  help="Output directory for eval results")
...
def load_model(save_dir: Path, num_domains: int, z_veh_dim: int,
               z_env_dim: int, device: torch.device):
    model = MultiModalCausalVAE(
    ...
    cls_head = ClassificationHead(
        z_veh_dim=z_veh_dim, num_classes=len(CLASS_MAP)
    ).to(device)
...
def main():
    ...
    # Use val dataset's domain count (model was trained on this)
    ...
    val_det_metrics = run_detection_eval(
        model, det_head, val_loader, device,
        eval_set_name="val", save_dir=eval_dir,
    )
    val_cls_metrics = run_classification_eval(
        model, cls_head, val_loader, device, save_dir=eval_dir,
    )
    ...
    iobt_det_metrics = run_detection_eval(
        model, det_head, iobt_loader, device,
        eval_set_name="iobt", save_dir=eval_dir,
    )
    print("  NOTE: Classification eval skipped for iobt "
    ...
    for eval_set, tasks in results.items():
        for task, m in tasks.items():
            ...
            print(f"  {eval_set:6s} {task:14s}  acc={m.get('accuracy', float('nan')):.4f}"
