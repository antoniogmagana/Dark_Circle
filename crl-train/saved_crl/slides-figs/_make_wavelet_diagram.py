...
def main() -> None:
    ...
    ax = axes[1]
    ax.plot(t_ms, np.cos(w0 * t / s), color=C_RE, label=r"$\cos(w_0 t / s)$")
    ax.plot(
        t_ms, np.sin(w0 * t / s), color=C_IM, label=r"$\sin(w_0 t / s)$", linestyle="--"
    )
    ...
    ax = axes[2]
    ax.plot(t_ms, gauss_n, color=C_ENV, linestyle=":", linewidth=1.6, label="envelope")
