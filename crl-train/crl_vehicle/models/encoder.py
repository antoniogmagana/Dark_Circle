...
Maps SSM output (B, T', d_model) to two task-specific embeddings via
separate projection branches. This eliminates gradient competition between
differently-sized latent blocks.
...
class MultiTaskEncoder(nn.Module):
    """
    Branched encoder producing two independent task embeddings.
    ...
    def forward(
        ...
        return self.proj_pres(h), self.proj_type(h)
