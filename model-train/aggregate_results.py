...
def main():
    ...
    df = df.sort_values(by=["Mode", "F1-Score", "MCC"], ascending=[True, False, False])
    ...
    # Filter to columns that actually exist in the dataframe to prevent errors with old runs
    ...
    for _index, row in df.iterrows():
        if row["Mode"] != current_mode:
            ...
            print(
                f"{'Model':<30} | {'Acc':<6} | {'F1':<6} | {'MCC':<6} | {'AUC':<6} | {'Latency':<10} | {'FAR':<6} | {'Timestamp':<25}"
            )
            print("-" * 105)
        ...
        print(
            f"{row['Model']:<30} | {acc_str:<6} | {f1_str:<6} | {mcc_str:<6} | {auc_str:<6} | {latency_str:<10} | {far_str:<6} | {time_str:<25}"
        )

    print("=" * 115)
    print(f"\nSaved full results (including Precision & Recall) to: {output_csv}\n")
