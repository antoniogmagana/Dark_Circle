...
class TestVehicleDatasetInit:
    ...
    def test_initialization(
        self, mock_samples, mock_align, mock_max_time, mock_tables, mock_config
    ):
...
class TestVehicleDatasetGetItem:
    ...
    def test_getitem_detection_mode(self, mock_config, *mocks):
        ...
        dataset = VehicleDataset(split="train", config=mock_config)
        dataset.samples = [("iobt", "mustang", "s1", 1, 0.0, "background")]
    ...
    def test_getitem_category_mode(self, mock_config, *mocks):
        ...
        dataset = VehicleDataset(split="train", config=mock_config)
        dataset.samples = [("iobt", "bicycle", "s1", 1, 0.0, "light")]
    ...
    def test_getitem_instance_mode(self, mock_config, *mocks):
        ...
        mock_config.INSTANCE_TO_CLASS = {"mustang": 1, "bicycle": 2, "background": 0}

        dataset = VehicleDataset(split="train", config=mock_config)
        dataset.samples = [("iobt", "mustang", "s1", 1, 0.0, "mustang")]
...
class TestVehicleDatasetSynthesis:
    ...
    def test_pure_synthetic_sample(self, mock_config, *mocks):
        ...
        dataset = VehicleDataset(split="train", config=mock_config)
        dataset.samples = [("synthetic", "background", None, None, None, "background")]
    ...
    def test_synthesize_background_flag(self, mock_config, *mocks):
        ...
        VehicleDataset(split="train", config=mock_config)

        assert mock_config.SYNTHESIZE_BACKGROUND is True
...
class TestDatasetIntegration:
    ...
    def test_multiple_training_modes(self, mock_config, *mocks):
        ...
        for mode in modes:
            ...
            assert mode == dataset.config.TRAINING_MODE
