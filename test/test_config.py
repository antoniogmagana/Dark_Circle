...
class TestModelSelection:
    ...
    def test_best_model_metric_valid(self):
        ...
        valid_metrics = ["val_acc", "val_loss", "val_f1", "val_precision", "val_recall"]
...
class TestDatasetConfiguration:
    ...
    def test_in_channels_matches_sensors(self):
        ...
        assert expected_channels == config.IN_CHANNELS
    ...
    def test_ref_sample_rate_is_maximum(self):
        ...
        max_native = max(
            config.NATIVE_SR[ds][s] for ds in config.TRAIN_DATASETS for s in config.TRAIN_SENSORS
        )
        assert max_native == config.REF_SAMPLE_RATE
